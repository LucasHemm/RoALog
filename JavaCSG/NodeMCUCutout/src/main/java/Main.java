...
//        result = csg.difference3D(result, leftsidesquareHole);
        ...
        Geometry3D innerCircle = roundSpool.getCutout(csg, 20, 15, 10, false);


        innerCircle = csg.rotate3DY(csg.degrees(90)).transform(innerCircle);
        innerCircle = csg.translate3D(-7.5, 0, 0).transform(innerCircle);

        Geometry3D innerbar = csg.cylinder3D(4, 4, 15, false);
        innerbar = csg.translate3D(0,0,10).transform(innerbar);

        Geometry3D result = csg.union3D(innerCircle, innerbar);

        Geometry3D innerBarTop = csg.cylinder3D(7, 1, 15, false);
        innerBarTop = csg.translate3D(0,0,13).transform(innerBarTop);

        result = csg.union3D(result, innerBarTop);


        Geometry3D rightSideCircle = roundSpool.getCutout(csg, 40, 1, 10, false);
        rightSideCircle = csg.rotate3DY(csg.degrees(90)).transform(rightSideCircle);
        rightSideCircle = csg.translate3D(7, 0, 0).transform(rightSideCircle);

        result = csg.union3D(result, rightSideCircle);

        Geometry3D leftSideCircle = roundSpool.getCutout(csg, 40, 1, 10, false);
        leftSideCircle = csg.rotate3DY(csg.degrees(90)).transform(leftSideCircle);
        leftSideCircle = csg.translate3D(-7.5, 0, 0).transform(leftSideCircle);
